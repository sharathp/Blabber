...
@ViewModelSpec(className="SearchWithUser", viewName="searches_and_tweets")
@Implements(interfaceClasses = {ITweetWithUser.class})
public class SearchWithUserSpec {

    @ViewQuery
    public static final Query QUERY = Query.select().from(Search.TABLE)
            .join(Join.inner(Tweet.TABLE, Search.TWEET_ID.eq(Tweet.ID)),
                    Join.inner(User.TABLE, Tweet.USER_ID.eq(User.ID)));

    public static final Property.StringProperty SEARCH_QUERY = Search.QUERY;
